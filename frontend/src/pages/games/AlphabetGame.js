      ...
      <div style={S.stageArea}>
        ...
        <div style={S.stagesGrid}>
          {STAGES.map((s, i) => {
            ...
            return (
              <motion.div key={i} style={{ ...S.stageCard, opacity:unlocked?1:0.5, border:stageIndex===i?'2px solid #8B5CF6':'1px solid #2D3A4F', background:unlocked?'rgba(139,92,246,0.1)':'#1E293B' }}
                whileHover={unlocked?{scale:1.05,borderColor:'rgba(139,92,246,0.5)'}:{}} whileTap={unlocked?{scale:0.95}:{}}
                onClick={() => { if (unlocked) { setStageIndex(i); setStageOver(false); } }}>
                <div style={{ fontSize:28 }}>{unlocked?'🔤':'🔒'}</div>
                <div style={{ fontSize:15, fontWeight:800, color:unlocked?'#8B5CF6':'#4B5563', fontFamily:'Nunito,sans-serif' }}>{s.name}</div>
                <div style={{ fontSize:14, letterSpacing:3, color:'#64748B', marginTop:4, fontFamily:'Nunito,sans-serif' }}>{s.letters.join(' ')}</div>
    ...
    <div style={S.page}>
      ...
      <div style={S.gameArea}>
        <motion.div style={S.questionBox} key={`${stageIndex}-${roundRef.current}`} initial={{ scale:0.8,opacity:0 }} animate={{ scale:1,opacity:1 }} transition={{ type:'spring',bounce:0.4 }}>
          ...
          <div style={{ fontSize:130, fontWeight:900, color:'#8B5CF6', lineHeight:1 }}>{targetLetter}</div>
        </motion.div>
...
const S = {
  ...
  backBtn:     { background:'rgba(99,102,241,0.12)', color:'#8B5CF6', border:'1px solid rgba(99,102,241,0.3)', padding:'8px 16px', borderRadius:12, fontSize:14, fontWeight:700, cursor:'pointer', fontFamily:'Nunito,sans-serif' },
  ...
  questionBox: { background:'#1E293B', border:'1px solid #2D3A4F', borderRadius:28, padding:'36px 60px', textAlign:'center', boxShadow:'0 8px 32px rgba(0,0,0,0.3)' },
  questionLabel:{ fontSize:16, color:'#94A3B8', fontWeight:700, margin:'0 0 12px', fontFamily:'Nunito,sans-serif' },
  ...
  startBtn:    { background:'linear-gradient(135deg,#8B5CF6,#EC4899)', color:'#fff', border:'none', padding:'16px 40px', borderRadius:18, fontSize:18, fontWeight:800, cursor:'pointer', fontFamily:'Nunito,sans-serif', boxShadow:'0 4px 20px rgba(139,92,246,0.4)' },
